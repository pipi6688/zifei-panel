...
const createExchange = (exchangeId) => {
  ...
  if (!supportedExchanges[normalizedId]) {
    throw new Error(
      `Unsupported exchange: ${exchangeId}. Supported exchanges: ${standardExchangeIds.join(
        ", "
      )}`
    );
  }
...
app.get("/api/all-swap-tickers", async (req, res) => {
  try {
    ...
    await Promise.all(
      standardExchangeIds.map(async (exchangeId) => {
        try {
          ...
          // 获取所有永续合约的ticker数据
          const tickers = await exchange.fetchTickers(undefined, {
            subType: "linear",
          });
...
app.get("/api/swap-tickers", async (req, res) => {
  try {
    ...
    if (exchange.toLowerCase() === "bitget") {
      ...
      if (
        instrumentsResponse.data.retCode === 0 &&
        instrumentsResponse.data.result.list
      ) {
      ...
      // 获取ticker数据
      tickers = await exchangeInstance.fetchTickers(undefined, {
        subType: "linear",
      });
      ...
      // 获取资金费率间隔数据
      const fundingInfoResponse = await axios.get(
    ...
    console.error(
      `Error fetching swap tickers for ${req.query.exchange}:`,
      error
    );
    res.status(500).json({
      success: false,
...
app.get("/api/funding-rates", async (req, res) => {
  try {
    ...
    if (exchange.toLowerCase() === "bitget") {
      // Bitget需要指定type参数
      fundingRates = await exchangeInstance.fetchFundingRates(undefined, {
        type: "swap",
      });
    } else if (exchange.toLowerCase() === "okx") {
      // OKX需要指定instType参数
    ...
    console.error(
      `Error fetching funding rates for ${req.query.exchange}:`,
      error
    );
    res.status(500).json({
      success: false,
      ...
      symbolList.map(async (symbol) => {
        try {
          ...
          if (
            ...
            const fundingRateInterval = Math.floor(
              (nextFundingTime - fundingTime) / 1000 / 60 / 60
            ); // 转换为小时
